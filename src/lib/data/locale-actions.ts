...
export const getLocaleForCountryCode = async (countryCode: string) => {
  const locale = await getLocale()
  return locale || getLocaleForCountry(countryCode)
}

export const getRequestLocale = async (): Promise<string | null> => {
  try {
    const headers = await nextHeaders()
    return headers.get("x-medusa-locale") || (await getLocale())
  } catch {
    return getLocale()
  }
}
